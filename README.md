...
This section validates end‑to‑end File Integrity Monitoring across multiple agents, including troubleshooting steps to ensure reliable, real‑time telemetry.
4.1 Windows FIM Lifecycle Test – Fistandantilus
Scenario
Validate that the Windows agent reports file add / modify / delete events in real time for a monitored path.
PowerShell Test Script
powershell
# Start filtered log tail in background
$job = Start-Job -ScriptBlock {
    Get-Content "C:\Program Files (x86)\ossec-agent\ossec.log" -Wait |
        Select-String -Pattern "added|modified|deleted|File"
}

Start-Sleep 1

# Run full FIM lifecycle test
...
# Retrieve filtered FIM events
Receive-Job $job -Keep
file:///screenshots/fim_demo_fistandantilus.png file:///screenshots/fim_alerts_fistandantilus.png file:///screenshots/fim_fistandantilus_example_json.png
Troubleshooting Summary
    • Agent logs initially showed invalid XML elements: merge and registry.
    • Removed unsupported <merge> and <registry> blocks from: /var/ossec/etc/shared/default/agent.conf
    • Restarted Wazuh Manager and Windows agent; confirmed clean startup.
...
4.2 Windows Registry FIM – Malystryx
Scenario
...
4.3 Linux FIM – Raistlin & DargaardKeep
Scenario
Validate Linux file integrity monitoring on /etc/cups/subscriptions.conf and related files.
file:///screenshots/fim_alert_raistlin.png file:///screenshots/fim_demo_raistlin.png
Observed Events
...
Troubleshooting Summary
    ...
    • Checked that the agent was not missing real‑time monitoring capabilities (inotify).
